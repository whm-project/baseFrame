            ...
            if(menuList.code == "0"){
                ...
                for(var i=0; i<menuList.data.length; i++){
                    ...
                    if(menuList.data[i].menuType != "3"){
                        ...
                        //设置新节点的显示内容
                        menu_item.find("span").html(menuList.data[i].menuNm);

                        //如果是页面，直接跳转
                        if(menuList.data[i].menuType == "2"){
                            menu_item.attr('onclick', 'openPage("'+menuList.data[i].menuCd+'")');
                        }

                        //设置子菜单的ul的id
                        menu_item.find("ul").attr("id", "secondLevel_menu_list_"+menuList.data[i].menuCd);
...
function init_click(){
    // nav收缩展开
    $('#menu_list').on('click','.nav-item>a',function(){
    ...
    $('#menu_list').on('mouseover','.nav-item',function(){
        if ($('.nav').hasClass('nav-mini')) {
